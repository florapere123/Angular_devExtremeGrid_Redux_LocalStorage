...
function updateRateItem(state: IRateItemState, action: any) {
    const e = action.payload;
     const ds = state.rateItems.map(item => item.id === e.id ? {
        ...item,
        ...e
    } : item);
...
function orderByDate(state: IRateItemState, action: any) {
  ...
  var rateItems=_.orderBy(state.rateItems, ['currentDate', 'Rate'], [action.payload.ByDate?'asc' : 'desc',action.payload.ByRate?'asc':'desc']);
   return {
    ...state,
    rateItems,
...
function orderByRate(state: IRateItemState, action: any) {
   var rateItems=_.orderBy(state.rateItems, ['Date', 'Rate'],[action.payload.ByDate ?'asc': 'desc' ,action.payload.ByRatee?'asc' : 'desc' ]);
   return {
     ...state,
     rateItems,
     sorting: action.payload,
